...
class RateLimiter:
    ...
    def sync_acquire(self):
        ...
        if len(self.calls) >= self.calls_per_minute:
            sleep_time = max(0.5, 60 - (now - self.calls[0]))  # Minimum 0.5s, max based on oldest call
...
class FinalMCPChat:
    def __init__(self):
        ...
        try:
            ...
            agent_arn = "arn:aws:bedrock-agentcore:us-east-1:975910639313:runtime/mcp_db_server_1755524390-HNauuT72Ex"
    ...
    def _discover_tools(self):
        ...
        if (self.available_tools and current_time < self.tools_cache_expiry) or self.tools_discovered:
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not response_text:
                    print("Warning: Empty response from server, using fallback static list")
                ...
                if not response_text:
                    print("Warning: Empty response after SSE parsing, using fallback static list")
                    self._use_fallback_tools()
                    return
                ...
                if 'result' in result and 'tools' in result['result']:
                    ...
                    print("No tools found in server response, using fallback static list")
                    self._use_fallback_tools()
            else:
                print(f"Warning: Server returned {response.status_code}, using fallback static list")
                self._use_fallback_tools()
                
        except json.JSONDecodeError as e:
            print(f"Warning: JSON decode error (likely server not supporting tools/list), using fallback static list")
            self._use_fallback_tools()
        except Exception as e:
    ...
    def _use_fallback_tools(self):
        ...
        print(f"Using {len(self.available_tools)} fallback tools (server doesn't support tools/list)")
    ...
    def call_mcp_tool(self, tool_name: str, arguments: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        cacheable_tools = {
            'list_tables', 'describe_table', 'get_database_info', 'health_check', 
            'get_business_module_tables', 'search_tables_by_pattern', 'list_available_tools'
        }
    ...
    def _parse_sse_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'data: ' in response_text:
                ...
                json_str = response_text[start_idx:end_idx] if end_idx != -1 else response_text[start_idx:].strip()
            else:
    ...
    def process_user_query_completely(self, user_input: str, conversation_history: List[Dict] = None, silent: bool = False, active_only: bool = True) -> str:
...
"""
        ...
        system_prompt = f"""You are a database assistant that EXECUTES ALL NECESSARY TOOLS AUTOMATICALLY.
...
The user asking for data IS the permission to execute everything needed. START EXECUTING NOW."""
            ...
            try:
                ...
                if not tool_calls:
                    if text_content:
                        ...
                        if any(keyword in response_text.lower() for keyword in ['session', 'found', 'data', 'result', 'record', 'total']):
                            ...
                            current_messages.append({
                                "role": "user", 
                                "content": [{"text": "Execute the necessary database tools to get the actual data. Do not ask for permission."}]
                            })
                            continue
                        else:
                            total_time = time.time() - start_time
                ...
                for result in tool_results:
                    result_text = result.get('toolResult', {}).get('content', [{}])[0].get('text', '')
                ...
                if has_data and iteration >= 2:  # Allow at least 2 iterations for discover + execute
                    continue
    ...
    def run_chat(self):
        ...
        print()
        print("Ask for any data - the system will automatically execute all needed tools!")
        print("Type 'help' for examples, 'silent' to toggle silent mode, 'active' to toggle active-only filter, or 'quit' to exit")
        ...
        while True:
            try:
                ...
                if user_input.lower() in ['quit', 'exit', 'q']:
                    ...
                    if active_only_mode:
                        print("   Active ON: Only shows active records (is_active = true)")
                    else:
                ...
                result = self.process_user_query_completely(user_input, conversation_history, silent=silent_mode, active_only=active_only_mode)
