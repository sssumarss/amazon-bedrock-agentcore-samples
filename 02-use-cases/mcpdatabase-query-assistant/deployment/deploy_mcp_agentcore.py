...
def check_python_version():
    ...
    if current_version < required_version:
        print(f"Python version {current_version[0]}.{current_version[1]} is not supported")
        print(f"   Please upgrade to Python {required_version[0]}.{required_version[1]} or higher")
        print(f"   Current version: {sys.version}")
        print(f"\nInstallation suggestions:")
        print(f"   macOS (Homebrew): brew install python@3.11")
        print(f"   Ubuntu/Debian: sudo apt install python3.11")
        print(f"   Windows: Download from https://python.org")
        sys.exit(1)
...
class CognitoSetup:
    ...
    def check_existing_setup(self):
        ...
        if self.config_file.exists():
            try:
                ...
                try:
                    self.cognito_client.describe_user_pool(UserPoolId=config['user_pool_id'])
                    print("Existing Cognito configuration found and verified")
                    return config
                except ClientError:
                    print(" Existing configuration found but user pool doesn't exist. Will recreate.")
                    return None
    ...
    def create_user_pool_client(self, user_pool_id, client_name="DatabaseMCPServerClient"):
    ...
    def create_test_user(self, user_pool_id, username="testuser@example.com", temp_password="TempPass123!"):
...
class EnhancedMCPDeployment:
    ...
    def setup_dockerfile_location(self):
        ...
        try:
            ...
            if dockerfile_source.exists():
                ...
                print("Created temporary Dockerfile in project root for toolkit compatibility")
                files_copied += 1
            ...
            if dockerignore_source.exists():
                ...
                print("Created temporary .dockerignore in project root for toolkit compatibility")
                files_copied += 1
    ...
    def setup_agentcore_config_location(self):
        ...
        try:
            ...
            if config_source.exists() and not config_target.exists():
                print("Copying AgentCore config from config/ to root for toolkit compatibility")
                import shutil
            ...
            # If both exist, ensure they're synchronized (config/ is master)
                ...
                if not (config_target.is_symlink() and config_target.resolve() == config_source.resolve()):
            ...
            if config_file.exists():
                ...
                if config and 'agents' in config:
                    ...
                    for agent_name, agent_config in config['agents'].items():
                        if (agent_name == self.server_name or 
                            agent_name.startswith(f"{self.server_name}_") or
                            agent_name.startswith(f"{self.server_name}_v")):
                            
                            # Check if this agent has a valid AWS ARN
                            if ('bedrock_agentcore' in agent_config and 
                                'agent_arn' in agent_config['bedrock_agentcore']):
                    ...
                    if existing_agents:
                        ...
                        if existing_name != self.server_name:
                            print(f"Renaming agent config from '{existing_name}' to '{self.server_name}'")
                            
                            # Copy the configuration to the standard name
                            config['agents'][self.server_name] = existing_config.copy()
                            config['agents'][self.server_name]['name'] = self.server_name
                        ...
                        print("Will reuse existing AWS agent (agent ARN remains unchanged)")
                        return existing_arn
    ...
    def deploy(self, force_rebuild=False):
        ...
        try:
            ...
            if existing_agent_arn:
                if force_rebuild:
                    print(f"🔄 Rebuilding Docker image for existing agent: {existing_agent_arn}")
                    ...
                    if config_file.exists():
                        ...
                        existing_agents = [(name, agent_config) for name, agent_config in config.get('agents', {}).items() 
                                         if 'bedrock_agentcore' in agent_config and 'agent_arn' in agent_config['bedrock_agentcore']]
                        
                        if existing_agents:
                            # Extract the actual agent name from the ARN
                            agent_arn = existing_agents[0][1]['bedrock_agentcore']['agent_arn']
                            # ARN format: arn:aws:bedrock-agentcore:us-east-1:975910639313:runtime/mcp_db_server_1755524390-HNauuT72Ex
                            existing_name = agent_arn.split('/')[-1].split('-')[0]  # Extract mcp_db_server_1755524390
                            ...
                            print("✅ Existing agent will use the new image on next invocation")
                    ...
                    self.display_deployment_summary(cognito_config, self.server_name, existing_agent_arn, rebuilt=True)
                    ...
                    self.display_deployment_summary(cognito_config, self.server_name, existing_agent_arn)
    ...
    def update_client_with_new_agent(self, agent_name):
        ...
        try:
            ...
            if config_file.exists():
                ...
                if ('agents' in config and agent_name in config['agents'] and 
                    'bedrock_agentcore' in config['agents'][agent_name] and
                    'agent_arn' in config['agents'][agent_name]['bedrock_agentcore']):
                    
                    new_arn = config['agents'][agent_name]['bedrock_agentcore']['agent_arn']
                    ...
                    if client_file.exists():
                        ...
                        replacement = f'agent_arn = "{new_arn}"  # Fresh agent with JWT support'
    ...
    def display_deployment_summary(self, cognito_config, agent_name=None, agent_arn=None, rebuilt=False):
        """Display deployment summary"""
        print("\n" + "=" * 60)
        ...
        print(f"   Protocol: MCP with JWT Authentication")
        print(f"   Database: PostgreSQL RDS")
        
        if agent_arn:
            print(f"   Agent ARN: {agent_arn}")
            if rebuilt:
                print(f"   Status: Rebuilt Docker image and updated existing agent")
            else:
                print(f"   Status: Reused existing agent")
        else:
            print(f"   Status: Created new agent")
        ...
        print(f"   JWT Token: Generated and saved")
        
        print(f"\nTesting:")
        print(f"   You can now test your MCP server with the generated JWT tokens")
        print(f"   Cognito configuration: ../config/cognito.json")
        print(f"   AgentCore configuration: ../config/.bedrock_agentcore.yaml")
        
        print(f"\nMonitoring:")
        print(f"   Check logs with: aws logs tail /aws/bedrock-agentcore/runtimes/{self.server_name}-*/DEFAULT --follow")
        
        print(f"\nNote:")
        if agent_arn:
            print(f"   Reused existing AWS agent - no new resources created")
            print(f"   JWT authorization updated for existing agent")
        else:
            print(f"   AWS agents retain their original ARN even when updated")
            print(f"   This is normal behavior - the agent functionality is updated correctly")
...
Examples:
  ...
  python3 deploy_mcp_agentcore.py --rebuild # Rebuild Docker image and update existing agent
    ...
    try:
        ...
        print(f"Failed to import AgentCore Runtime toolkit: {e}")
        print("Please install the required dependencies:")
        print("   pip install bedrock_agentcore_starter_toolkit")
        sys.exit(1)
    
    parser = argparse.ArgumentParser(description='Enhanced MCP Server Deployment with Cognito Setup')
    parser.add_argument('--server-name', default='mcp_db_server', 
